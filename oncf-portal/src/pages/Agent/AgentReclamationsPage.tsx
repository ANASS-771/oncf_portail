...
import { getAgentReclamations, getMyProfile, getReclamationAudit, updateReclamation } from '../../api/client';
import type { CurrentUserProfile, PagedResult, Reclamation, ReclamationAuditEntry, UpdateReclamationPayload } from '../../types';
import {
  ChevronLeft, ChevronRight, Clock, HeadphonesIcon, MessageSquare, Search, User, X,
} from 'lucide-react';
...
function StatutBadge({ statut }: Readonly<{ statut: string }>) {
...
function TreatModal({ rec, onClose, onSaved }: Readonly<TreatModalProps>) {
  ...
  const [form, setForm] = useState<UpdateReclamationPayload>({ statut: 'RESOLUE', reponseAdmin: '' });
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      notify({ type: 'success', title: 'Réclamation clôturée', message: `Statut : ${STATUT_LABEL[form.statut] ?? form.statut}` });
      onSaved();
    } catch {
        ...
        <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <button type="submit" disabled={loading} style={{
              ...
              background: '#4338ca', cursor: loading ? 'not-allowed' : 'pointer',
              fontSize: 13, fontWeight: 700, color: 'white', opacity: loading ? 0.7 : 1,
            }}>
              {loading ? 'Enregistrement...' : 'Clôturer la réclamation'}
            </button>
          </div>
        </form>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {audit.map(e => (
                <div key={e.auditId} style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                  <div style={{ width: 28, height: 28, borderRadius: 14, background: '#eef2ff', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
...
export default function AgentReclamationsPage() {
  ...
  useEffect(() => { getMyProfile().then(setProfile).catch(console.error); }, []);

  const load = () => {
    setLoading(true);
    setError(false);
    getAgentReclamations(page, PAGE_SIZE, statut || undefined, search || undefined)
      .then(setResult)
      .catch(() => setError(true))
      .finally(() => setLoading(false));
  };

  useEffect(() => { load(); }, [page, statut, search]);

  const myLogin = profile?.login ?? '';

  const handleTakeOver = async (rec: Reclamation) => {
    setTakingOver(rec.id);
    try {
      await updateReclamation(rec.id, { statut: 'EN_COURS' });
      notify({ type: 'success', title: 'Prise en charge', message: `Réclamation ${rec.numeroReference} assignée.` });
      load();
    } catch {
      notify({ type: 'error', title: 'Erreur', message: 'Impossible de prendre en charge cette réclamation.' });
    } finally {
      setTakingOver(null);
    }
  };
  ...
  const items = tab === 'mes'
    ? allItems.filter(rec => rec.assignedToLogin === myLogin)
    : allItems;
    ...
    <div className="page">
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
        <div style={{ width: 40, height: 40, borderRadius: 10, background: '#eef2ff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <HeadphonesIcon size={18} color="#4338ca" />
        </div>
        <div>
          <h1 style={{ fontSize: 18, fontWeight: 800, color: 'var(--gray-900)' }}>Réclamations</h1>
          <p style={{ fontSize: 12, color: 'var(--gray-400)', marginTop: 2 }}>
            {result ? `${result.totalCount} réclamation${result.totalCount > 1 ? 's' : ''}` : '—'}
          </p>
        </div>
      </div>

      {/* Tabs */}
      <div style={{ display: 'flex', gap: 4, marginBottom: 16, borderBottom: '2px solid var(--gray-100)' }}>
        {([['tous', 'Toutes'], ['mes', 'Mes réclamations']] as [TabFilter, string][]).map(([val, label]) => (
          <button key={val} onClick={() => setTab(val)} style={{
            padding: '8px 16px', fontSize: 13, fontWeight: 600, border: 'none', background: 'none', cursor: 'pointer',
            color: tab === val ? '#4338ca' : 'var(--gray-500)',
            borderBottom: `2px solid ${tab === val ? '#4338ca' : 'transparent'}`,
        ...
        <form onSubmit={handleSearch} style={{ display: 'flex', gap: 6 }}>
          <div style={{ position: 'relative' }}>
            ...
            <input value={searchInput} onChange={e => setSearchInput(e.target.value)}
              placeholder="Client, objet, conteneur..."
              style={{ padding: '7px 10px 7px 28px', borderRadius: 7, border: '1px solid var(--gray-200)', fontSize: 13, width: 240 }} />
          </div>
          <button type="submit" style={{
            padding: '7px 14px', borderRadius: 7, border: 'none', background: '#4338ca',
      ...
      {loading ? <PageSpinner /> : items.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '60px 0', color: 'var(--gray-400)' }}>
          ...
          <p style={{ fontSize: 14 }}>
            {tab === 'mes' ? 'Aucune réclamation assignée' : 'Aucune réclamation trouvée'}
          </p>
        </div>
      ) : (
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            ...
            <tbody>
              {items.map(rec => {
                const isAssignedToMe = myLogin && rec.assignedToLogin === myLogin;
                const canTakeOver = rec.statut === 'EN_ATTENTE';
                const canTreat = rec.statut === 'EN_COURS' && isAssignedToMe;
                ...
                return (
                  ...
                  >
                    ...
                    <td style={{ padding: '10px 12px' }}>
                      {rec.assignedToLogin ? (
                        <span style={{
                          ...
                          background: isAssignedToMe ? '#eef2ff' : '#f3f4f6',
                          color: isAssignedToMe ? '#4338ca' : 'var(--gray-600)',
                        }}>
                          <User size={10} />
                          {isAssignedToMe ? 'Moi' : rec.assignedToLogin}
                        </span>
                      ) : (
                    ...
                    <td style={{ padding: '10px 12px' }}>
                      {canTakeOver && (
                        <button
                          onClick={() => handleTakeOver(rec)}
                          disabled={takingOver === rec.id}
                          style={{
                            padding: '5px 12px', borderRadius: 6, border: '1px solid #a5b4fc',
                            background: '#eef2ff', color: '#4338ca',
                            fontSize: 12, fontWeight: 700, cursor: takingOver === rec.id ? 'wait' : 'pointer',
                            opacity: takingOver === rec.id ? 0.6 : 1,
                          }}
                        >
                          Prendre en charge
                        </button>
                      ...
                      {rec.statut === 'EN_COURS' && !isAssignedToMe && rec.assignedToLogin && (
                        <span style={{ fontSize: 11, color: 'var(--gray-400)' }}>En cours…</span>
                      )}
                    </td>
                  </tr>
                );
      ...
      {/* Pagination — only in "tous" tab since "mes" is a client-side filter */}
